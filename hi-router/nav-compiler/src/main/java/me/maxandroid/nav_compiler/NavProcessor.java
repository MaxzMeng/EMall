package me.maxandroid.nav_compiler;
...
import me.maxandroid.nav_annotation.Destination;

@AutoService(Processor.class)
@SupportedSourceVersion(SourceVersion.RELEASE_8)
@SupportedAnnotationTypes({"me.maxandroid.nav_annotation.Destination"})
public class NavProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        messager.printMessage(Diagnostic.Kind.NOTE, elements.size() + "");
        if (!elements.isEmpty()) {
    ...
    private void handleDestination(Set<? extends Element> elements, Class<Destination> destinationClass, HashMap<String, JSONObject> destMap) {
        for (Element element : elements) {
            ...
            boolean asStater = annotation.asStarter();
            int id = Math.abs(clazzName.hashCode());

            String destType = getDestinationType(typeElement);

            if (destMap.containsKey(pageUrl)) {
                messager.printMessage(Diagnostic.Kind.ERROR, "there are two same pageUrl:" + pageUrl);
            } else {
                JSONObject jsonObject = new JSONObject();
                jsonObject.put("clazName", clazzName);
                jsonObject.put("pageUrl", pageUrl);
                jsonObject.put("asStater", asStater);
